...
#required settings 
filename=F2sWithParentsfreebayes
numsamples=35

#tunable parameters
MinDepthPerSampleAverage=6 # on average, at least how many reads should be present per individual
MaxDepthPerSampleAverage=30 # on average, what is the upper limit for reads per individual
MinQual=998 #minimum site quality
mac=3 #minor allele count
minGQ=30 # what is the minimum quality for a call to be kept
MinDP=6 # minimum depth per sample for that call to be retained
...
vcftools --vcf $filename.vcf --max-missing $MaxMissing --minDP $MinDP --mac $mac --minGQ $minGQ --recode --recode-INFO-all --out $filename.GQ.vcf
...
bcftools filter -o $filename.DPGQ.vcf -e 'DP < '$mindp' || DP > '$maxdp' || QUAL < '$MinQual $filename.GQ.vcf
bcftools stats $filename.DPGQ.vcf > $filename.DPGQ.vcf.stats

#remove any loci with allele balance below 25%, above 75% or about 0. Following lines pulled from http://www.ddocent.com/filtering/
